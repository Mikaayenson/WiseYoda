# Graciously taken from https://gist.github.com/colindean/9682b62ef5b3e87028bb8e23b5f4c4aa
# Set this to ~use it everywhere in the project setup
PYTHON_VERSION ?= 3.10

# the URL to use for PyPI proxying
PYPI_PROXY ?= https://pypi.org/simple
# the directories containing the library modules this repo builds
LIBRARY_DIRS = wise_yoda
...
check-py-flake8: ## Runs flake8 linter
	${RUN_PYPKG_BIN} flake8 tests wise_yoda --count --select=E9,F63,F7,F82 --show-source --statistics
	${RUN_PYPKG_BIN} flake8 tests wise_yoda --count --max-complexity=10 --max-line-length=120 --statistics

.PHONY: check-py-black
check-py-black: ## Runs black in check mode (no changes)
	$(RUN_PYPKG_BIN) black wise_yoda --check --line-length 120 --fast .
